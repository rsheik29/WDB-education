...
let total = 0;
let strbuffer = "0";
document.getElementById("result-screen").innerHTML = strbuffer;
let operator = "+";
...
function calculations() {
    const intBuffer = parseInt(buttonClicked); // Hint: Use parseInt to convert string to integer
    if (operator === "+") {
        total += intBuffer;
    } else if (operator === "-") {
        total -= intBuffer;
...
//Hint: We want to select all buttons from html and make it so that something happens when you click on the buttons! querySelectorAll might be helpful
    ...
    for (item of buttonList) {
        addEventListener('click', buttonClicked(item)); 
